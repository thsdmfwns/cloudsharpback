...
namespace cloudsharpback.Services.Interfaces
{
    public interface IShareService
    {
        ...
        Task<HttpErrorDto?> CloseShareAsync(MemberDto member, string token);
        Task<HttpErrorDto?> DeleteShareAsync(string target, MemberDto member);
        /// <returns>404 : share doesnt exist , 403 : bad password, 410 : expired share</returns>
        Task<(HttpErrorDto? err, ShareDownloadDto? dto)> GetDownloadDtoAsync(ShareDowonloadRequestDto req);
        ...
        Task<HttpErrorDto?> UpdateShareAsync(ShareUpdateDto dto, string token, MemberDto member);
